...
class ReminderListDataSource: NSObject {
    ...
    var percentComplete: Double {
        guard self.filteredReminders.count > 0 else {
            return 1
        }
        let numComplete: Double = self.filteredReminders.reduce(0) { $0 + ($1.isComplete ? 1 : 0) }
        return numComplete / Double(self.filteredReminders.count)
    }

    private var filter: Filter = .today
...
private extension ReminderListDataSource {
    @objc
